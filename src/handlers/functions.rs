...
fn rule_call_expression(token: &mut Token, state: &mut ParserState) -> Option<ParserError> {
    ...
    if state.extensions.has_function(name) {
        match state.extensions.call_function(name, token, &args, &mut state.variables) {
            Ok(v) => {
                token.set_value(v);
                return None
            },
    ...
    if let Some(f) = state.user_functions.get(name) {
        if args.len() != f.arguments().len() {
            return Some(FunctionNArgsError::new(token, f.name(), f.arguments().len(), f.arguments().len()).into())
            ...
            return Some(StackError::new(token).into())
        }
    }

    Some(FunctionNameError::new(token, name).into())
}
...
mod test_token {
    ...
    fn test_builtin_function_call() {
        ...
        assert_token_error!("sqrt('string')", FunctionArgType);
        assert_token_error!("sqrt()", FunctionNArgs);
    ...
    fn test_user_function_call() {
        ...
        assert_token_text_stateful!("fn(x, y) = 5x + 10(x * y)\nfn(2, 3)", "5x + 10(x * y)\n70", &mut state);
        assert_token_error!("f(x) = f(x)\nf(0)", Stack);
        assert_token_text_stateful!("sum(a) = element(a, 0) + ( len(a)>1 ? sum(dequeue(a)) : 0 )", "element(a, 0) + ( len(a)>1 ? sum(dequeue(a)) : 0 )", &mut state);
