...
pub fn handler_table() -> HashMap<Rule, RuleHandler> {
    HashMap::from([
        (Rule::bool_cmp_expression, rule_bool_cmp_expression as RuleHandler),
        (Rule::bool_and_expression, rule_bool_and_expression as RuleHandler),
        (Rule::bool_or_expression, rule_bool_or_expression as RuleHandler),
    ])
}
...
/// x < 3
/// x == 3
fn rule_bool_cmp_expression(token: &mut Token, _state: &mut ParserState) -> Option<ParserError> {
        ...
        token.set_value(
            Value::Boolean(
                match token.child(i+1).unwrap().rule() {
                    ...
                    Rule::le => l.le(&r),
                    _ => {
                        return Some(InternalError::new(token).into())
                    }
                }
            )
        );
...
/// a && b
fn rule_bool_and_expression(token: &mut Token, _state: &mut ParserState) -> Option<ParserError> {
    let mut i = 0;
    token.set_value(token.child(i).unwrap().value());
    while i < token.children().len() - 2 {
...
/// a || b
fn rule_bool_or_expression(token: &mut Token, _state: &mut ParserState) -> Option<ParserError> {
    let mut i = 0;
    token.set_value(token.child(i).unwrap().value());
    while i < token.children().len() - 2 {
        token.set_value(Value::Boolean(token.value().as_bool() || token.child(i+2).unwrap().value().as_bool()));
...
#[cfg(test)]
mod test_token {
    use crate::{Value, test::assert_token_value};
    use super::*;
    use crate::test::*;
