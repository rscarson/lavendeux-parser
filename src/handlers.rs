...
use crate::{
    token::{Rule, Token, OutputFormat, LavendeuxHandler},
...
impl LavendeuxHandler for Handler {
    fn handle_tree(&self, token: &mut Token, state: &mut ParserState) -> Result<(), ParserError> {
        ...
        if token.rule() == Rule::function_assignment {
            ...
            // Store new function
            state.user_functions.insert(name.to_string(), 
        ...
        for child in token.children() {
            if child.value().is_identifier() {
                let err = VariableNameError::new(child, &child.value().to_string());
    
                // Help function is allowed to have an unresolved identifier
                if !(token.rule() == Rule::call_expression && token.child(0).unwrap().text() == "help") {
                    return Err(err.into());
                }
            }
        }
...
type RuleHandler = fn(token: &mut Token, state: &mut ParserState) -> Option<ParserError>;
fn handler_table() -> HashMap<Rule, RuleHandler> {
    HashMap::from([
        ...
        (Rule::assignment_expression, rule_assignment_expression as RuleHandler),
    ]).into_iter()
...
/// A series of lines
fn rule_script(token: &mut Token, _state: &mut ParserState) -> Option<ParserError> {
...
fn rule_line(token: &mut Token, state: &mut ParserState) -> Option<ParserError> {
    ...
    if token.value().is_identifier() {
        return Some(VariableNameError::new(token.child(0).unwrap(), token.child(0).unwrap().text()).into());
    }
...
/// expression
/// ( expression )
fn rule_term(token: &mut Token, _state: &mut ParserState) -> Option<ParserError> {
...
fn rule_assignment_expression(token: &mut Token, state: &mut ParserState) -> Option<ParserError> {
    ...
    /// identifier[index] = expression
    fn rule_assignment_expression_indexed(token: &mut Token, state: &mut ParserState) -> Option<ParserError> {
        ...
        if let Some(value) = state.variables.clone().get(identifier) {
            match value.clone() {
                Value::Object(mut v) => {
                    ...
                    state.variables.insert(identifier.to_string(), Value::Object(v));
                ...
                _ => {
                    match index.as_int() {
                        Some(i) => {
                            ...
                            state.variables.insert(identifier.to_string(), Value::Array(array));
                            token.set_value(result);
                        },
    ...
    /// identifier = expression
    fn rule_assignment_expression_variable(token: &mut Token, state: &mut ParserState) -> Option<ParserError> {
        ...
        if state.constants.contains_key(identifier.text()) {
            ...
            return Some(ConstantValueError::new(token, identifier.text()).into())
        } else {
            // Update value
            state.variables.insert(identifier.text().to_string(), token.child(1).unwrap().value());
            token.set_value(token.child(1).unwrap().value());
        }
