...
const CONCAT: FunctionDefinition = FunctionDefinition {
    ...
    handler: |_function, _token, _state, args| {
        Ok(Value::String(
            args.iter().map(|v| v.as_string()).collect::<String>(),
        ))
    },
};
...
const STRLEN: FunctionDefinition = FunctionDefinition {
    ...
    arguments: || vec![FunctionArgument::new_required("s", ExpectedTypes::String)],
    handler: |_function, _token, _state, args| {
...
mod test_builtin_functions {
    ...
    fn test_concat() {
        ...
        assert_eq!(
            Value::String("test4false".to_string()),
            CONCAT
                .call(
                    &Token::dummy(""),
                    &mut state,
