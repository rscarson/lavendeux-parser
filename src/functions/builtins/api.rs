...
use super::*;
use crate::{network::ApiInstance, ExpectedTypes, Value};
...
const CALL: FunctionDefinition = FunctionDefinition {
    ...
    handler: |_function, token, state, args| {
        ...
        let endpoint = args
            .get("endpoint")
            .optional_or(Value::String("".to_string()))
            .as_string();
        ...
        match state.apis.get(&api_name) {
            Some(api) => {
                match api.request(
                    ...
                    Ok(result) => Ok(Value::String(result.as_string())),
                    Err(e) => Err(Error::Network(e, token.clone())),
                }
            }

            None => Err(Error::UnknownApi {
                name: api_name,
                token: token.clone(),
            }),
        }
...
mod test_builtin_functions {
        ...
        assert_eq!(
            ...
            REGISTER
                .call(
                    &Token::dummy(""),
                    &mut state,
                    &[Value::String(name.clone()), Value::String(url)]
                )
                .unwrap()
                .as_string()
                .contains(&name)
    ...
    fn test_list() {
        ...
        assert_eq!(
            true,
            LIST.call(&Token::dummy(""), &mut state, &[])
                .unwrap()
                .as_string()
                .contains(&name)
        );
